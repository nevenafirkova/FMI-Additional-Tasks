...
public class PinValidation
{
    static void Main()
    {
        ...
        //OUTPUT
        if (PinChecker(input, pinInput, out genderString, out birthArea, out bornDate))
        {
    ...
    //ACTION
    static bool PinChecker(string input, char[] pin, out string genderString, out string birthArea, out string bornDate)
    {
        bool isValid = true;

        genderString = string.Empty;
        birthArea = string.Empty;
        bornDate = string.Empty;
        ...
        int yearFourDigits = 0;
        if (month >= 40 && month < 50)
        {
            month -= 40;
            yearFourDigits = 2000 + year;
        }
        else if (month >= 50 && month <= 52)
        {
            month -= 40;
            yearFourDigits = 2000 + year;
        }
        ...
        {
            month -= 20;
            yearFourDigits = 1800 + year;
        }
        //Kids Born Before 2000 and after 1899
        else
        {
            yearFourDigits = 1900 + year;
        }
        ...
        {
            DateTime dt = new DateTime(yearFourDigits, month, day);
            bornDate = String.Format("{0:dddd, MMMM d, yyyy}", dt);
        }

        //BIRTH AREA
        string birthAreaNumbersString = input.Substring(6, 3);
